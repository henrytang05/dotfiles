#!/usr/bin/env zsh
set -e
echo -e "export ZDOTDIR=\"\$HOME/.config/zsh\"" >>~/.zprofile
...
# Loop through all files in ./scripts and skip those containing "macos"
for file in ./scripts/*; do
    if [[ -f "$file" ]]; then
        # Skip files containing "macos" (case-insensitive)
        if [[ "$file" =~ [Mm][Aa][Cc][Oo][Ss] ]]; then
            echo "Skipping $file (macOS file)"
            continue
        fi
